...
namespace GoSharp
{
    ...
    public class Game
    {
        ...
        /// </summary>
        public static readonly SuperKoComparer SuperKoComparer = new();

        private static readonly Dictionary<string, Content> SGFPropToColor = new()
        {
            { "AE", Content.Empty },
            { "AB", Content.Black },
            { "AW", Content.White }
        ...
        private static readonly Dictionary<Content, string> ColorToSGFProp = [];
        private static readonly Dictionary<string, Func<Game, SGFProperty, Game>> PropertyHandlers = [];

        private static readonly HashSet<string> PropertiesToExclude = ["W", "B", "AE", "AB", "AW"];
        ...
        private readonly HashSet<Board> _superKoSet = new(SuperKoComparer);
        private readonly List<SGFProperty> _sgfProperties = [];
        private Dictionary<Content, Group>? _setupMoves = null;
...
#pragma warning restore CS8618 // null 非許容のフィールドには、コンストラクターの終了時に null 以外の値が入っていなければなりません。Null 許容として宣言することをご検討ください。
        ...
        protected bool InternalMakeMove(int x, int y)
        {
            ...
            else _captures[oturn] += Board.Capture(capturedGroups.Where(p => p.Content == oturn.Opposite()));
            if (_superKoSet != null)
            {
                if (_superKoSet.Contains(Board, SuperKoComparer)) // Violates super-ko
                    legal = false;
        ...
        {
            using var sr = new StreamReader(path, ASCIIEncoding.ASCII);
        ...
        private static void CreateGameTree(SGFGameTree root, Game p)
        {
            if (p.GameInfo != null)
            {
                ...
                {
                    if (PropertyHandlers.TryGetValue(m.Name, out Func<Game, SGFProperty, Game>? value))
                        value(p, m);
